...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
				    ...
				    html += '							<img class="upload_image" src="'+path+'/resources/js/zyupload/skins/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			 ...
			 * 返回: 通过的文件
			 */
			this.funFilterEligibleFile = function(files,selFiles){
				var length=0;
				for(var i=0;i<para.fileInputs.length;i++){
		    		if(para.fileInputs[i].value==''||para.fileInputs[i].value==null){
		    			length++;
		    		}
		    	}
				var arrFiles = [];  // 替换的文件数组
				//判断文件个数是否达到上限							
				if((files.length+selFiles.length-1)>=length){
					//alert('您选中的文件个数已经达到上限');
					myAlert("只允许文件总数为"+para.fileInputs.length+"份","warning");
				}else{
						...
						if(newStr.split(".")[0] != null){
							...
							if(jQuery.inArray(type.toLowerCase(), para.fileType) > -1){							
									...
									if (file.size >= para.fileSize) {
										...
										myAlert(file.name +'"文件大小过大',"warning");
									} else {
										// 在这里需要判断当前所有文件中
										arrFiles.push(file);	
									}
							}else{
								console.debug(para.fileType)
								//alert('文件"'+ file.name +'"上传类型不符合');	
								myAlert('只能上传'+para.fileType+'格式文件',"warning");
							}
						}else{
							//alert('文件"'+ file.name +'"没有类型, 无法识别');	
							myAlert('文件"'+ file.name +'"没有类型, 无法识别',"warning");
						}
					}
				}				
				return arrFiles;
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = path+"/resources/js/zyupload/skins/images/fileType/";
				if(type == "rar"){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + e.target.result + '" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width);max-height:'+imgHeight+'px;" />';                                                                 
					html += '		</div>';
					html += '	</a>';
			...
			this.createCorePlug = function(){				
				var params = {
					...
					filterFile: function(files) {						
						// 过滤合格的文件
						return self.funFilterEligibleFile(files,ZYFILE.funReturnNeedFiles());
					},
...
//								}
					...
					onDelete: function(file, files) {
						...
						//清空保存图片路径的隐藏表单域
						for(var i=0;i<para.fileInputs.length;i++){
				    		if(para.fileInputs[i].value==$("#uploadList_" + file.index).attr("alt")){
				    			para.fileInputs[i].value="";
				    		}
				    	}
						$("#uploadList_" + file.index).remove();
						var listfile=$(".upload_append_list");
						//para.fileCount=para.fileCountBak-$(".upload_append_list").length;
						// 重新设置统计栏信息
					...
					onSuccess: function(file, response) {
						//alert(ZYFILE.funReturnNeedFiles.length);
						var fileUrl=para.onSuccess(file, response);// 回调方法	
						$("#uploadList_" + file.index).attr("alt",fileUrl);
						$("#uploadProgress_" + file.index).hide();
						$("#uploadFailure_" + file.index).hide();
						$("#uploadSuccess_" + file.index).show();
						//$("#uploadInf").append("<p>上传成功，文件地址是：" + response + "</p>");
						// 根据配置参数确定隐不隐藏上传成功的文件						
						//para.fileCount=para.fileCount-1;
						var listfile=$(".upload_append_list");
						//para.fileCount=para.fileCountBak-$(".upload_append_list").length;
						//alert(para.fileCount);
						if(para.finishDel){
							// 移除效果
							//$("#uploadList_" + file.index).fadeOut();
							$("#uploadList_" + file.index).remove();
